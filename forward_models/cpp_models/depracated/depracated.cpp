...
//won't work because using placement new with Eigen::Matrices and std::vectors or Eigen::Maps tries to free memory too many times
void forward_prop::get_batches(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & x_tr_b, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> & y_tr_b) {
    ...
    if (m == batch_size) {
        ...
        new (&x_tr_b) Eigen::Map< Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> >(x_tr_m);
        new (&y_tr_b) Eigen::Map< Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> > (y_tr_m);
    }
...
//old version of forward_prop class from scratch_gauss_slp.cpp
forward_prop::forward_prop(uint num_inputs_, uint num_outputs_, uint m_, uint batch_size_, std::vector<uint> layer_sizes_, std::string x_path_, std::string y_path_) : 
    ...
    // using this new method for maps causes seg fault from memory leak, 
    // so it appears that the vector and the map don't free the memory correctly.
}
