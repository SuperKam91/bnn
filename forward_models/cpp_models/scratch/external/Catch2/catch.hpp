...
namespace Matchers {
    ...
    namespace Vector {
        ...
        template<typename T> struct default_equals { bool operator()(T lhs, T rhs) {return lhs==rhs;} };

        template<typename T, typename C=default_equals<T>>
        ...
        template<typename T> struct approx_equals { bool operator()(T lhs, T rhs) {return lhs==Catch::Detail::Approx(rhs);} };

        template<typename T>
        struct ApproxMatcher : EqualsMatcher<T, approx_equals<T>> {
            ApproxMatcher(std::vector<T> const &comparator) : EqualsMatcher<T, approx_equals<T>>( comparator ) {}
            std::string describe() const override {
                return "Approx: " + ::Catch::Detail::stringify( EqualsMatcher<T, approx_equals<T>>::m_comparator );
            }
        };
