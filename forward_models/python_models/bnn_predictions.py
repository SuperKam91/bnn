...
class bnn_predictor():
	...
	def y_test_prob(self, test_index = 0):
		...
		y_test_prob = np.sum(test_LL * self.posterior_weights)
		return y_test_prob

	def y_custom_prob(self, y):
		"""
		as y_test_prob(), but takes y as argument, so don't have to use true value
		"""
		LL = self.get_test_LL(x, y)
		y_prob = np.sum(LL * self.posterior_weights)
		return y_prob
	...
	def get_pred_arr(x):
		...
		"""
		pred = np.zeros((len(self.posterior_weights), y.shape()[1]))
		for i in range(len(self.posterior_weights)):
			self.set_k_weights(self.nn_param_sets[i])
